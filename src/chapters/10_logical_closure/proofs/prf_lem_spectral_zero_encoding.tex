...
From the determinant identity in \thmref{thm:det_identity_revised}, we have:
\[
\det\nolimits_\zeta(I - \lambda L_{\sym}) = \frac{\XiR\left( \tfrac{1}{2} + i\lambda \right)}{\XiR\left( \tfrac{1}{2} \right)},
\]
where \( \XiR(s) \) is the completed Riemann zeta function. The zeros of the determinant coincide with the zeros of \( \XiR(\tfrac{1}{2} + i\lambda) \), and their multiplicities are preserved via Hadamard factorization, as described in \lemref{lem:hadamard_linear_form}. The normalization at \( \lambda = 0 \) follows from \lemref{lem:trace_zero}.
