...
\begin{itemize}
    ...
    \phi \in \PW{\pi},
    \]
    the Paley--Wiener class of entire functions whose Fourier transforms are supported in \( [-\pi, \pi] \); see \defref{def:paley_wiener_class} and \cite[Thm.~3.7.1]{Levin1996EntireLectures}.
...
\]
belong to the Schwartz space \( \Schwartz \), and their inverse Fourier transforms \( k_t := \FT^{-1}[\phi_t] \) decay exponentially in space, as proven in \lemref{lem:decay_mollified_kernel}, and support trace-class inclusion of kernels as used in \lemref{lem:decay_inverse_fourier_xi} and \lemref{lem:weighted_L1_inverse_FT_xi}.
\end{lemma}
