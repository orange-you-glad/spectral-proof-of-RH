...
\]
where \( \Xi(s) \) is the completed Riemann zeta function—entire of order one and exponential type \( \pi \) (see \lemref{lem:xi_growth_bound}).
...
\begin{enumerate}
    ...
    \]
    This follows from the Paley--Wiener bound \( \phi \in \PW{\pi} \) (see \defref{def:paley_wiener_class}) and decay strengthened by Gaussian mollification.
...
\]
lies in the trace class \( \TC(\HPsi) \), as confirmed in \lemref{lem:trace_class_Lt} via Simon’s kernel criterion~\cite[Thm.~4.2]{Simon2005TraceIdeals}.
