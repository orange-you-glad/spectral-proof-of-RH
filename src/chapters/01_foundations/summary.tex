...
\begin{itemize}
  ...
  \item \lemref{lem:decay_mollified_kernel}, \lemref{lem:L1_integrability_conjugated_kernel}, \lemref{lem:mollified_profile_decay} — Mollifiers \( k_t \in \Schwartz \), with exponential decay and integrability.
  ...
  \item \lemref{lem:unitary_conjugation_trace_class}, \lemref{lem:kernel_L2_weighted_bound} — Trace norm preserved under unitary weight conjugation; weighted \( L^2 \)-kernel estimates.
\end{itemize}
...
This chapter establishes the analytic and operator-theoretic base for all that follows. The canonical convolution operator \( \Lsym \in \TC(\HPsi) \) is defined as the trace-norm limit of mollified Fourier convolution operators \( L_t \), as shown in \lemref{lem:trace_norm_convergence_Lt_to_Lsym}. Its construction relies on Paley--Wiener estimates, exponential decay (\lemref{lem:mollified_profile_decay}), Sobolev density, and trace-class embedding theorems (\lemref{lem:trace_class_Lt}). A structurally critical threshold emerges: \( \alpha > \pi \) is both necessary and sufficient for trace-class inclusion, marking the sharp boundary for spectral realization (\lemref{lem:trace_class_failure_alpha_leq_pi}). The determinant identity
\[
\detz(I - \lambda \Lsym)
= \frac{\Xi\left(\tfrac{1}{2} + i\lambda \right)}{\Xi\left(\tfrac{1}{2} \right)}
\]
is proven in \thmref{thm:det_identity_revised}, resting entirely on this analytic groundwork.
