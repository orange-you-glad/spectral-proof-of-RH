\begin{proof}[Proof of \thmref{thm:existence-Lsym}]
...
By \lemref{lem:trace-norm-convergence-Lt-to-Lsym}, the family \( \{L_t\}_{t > 0} \) converges in trace norm:
\[
...
By \lemref{lem:core-essential-selfadjointness}, the restriction of \( L_{\mathrm{sym}} \) to \( \mathcal{S}(\mathbb{R}) \) is essentially self-adjoint, and its closure defines a unique self-adjoint operator:
\[
L_{\mathrm{sym}} = L_{\mathrm{sym}}^*.
\]

\paragraph{(v) Trace Normalization.}
By \thmref{thm:trace-zero-Lsym}, the trace of \( L_{\mathrm{sym}} \) vanishes:
\[
