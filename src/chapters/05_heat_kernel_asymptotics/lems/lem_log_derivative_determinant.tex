...
\]
as described in \lemref{lem:heat_trace_expansion} and derived from the pointwise kernel structure in \lemref{lem:hk_expansion_uniform}.
...
\]
as given in \lemref{lem:spectral_zeta_from_heat}, combined with analytic subtraction of \( P(t) \) near \( t = 0 \) to ensure convergence at \( s = 0 \). The regularized logarithmic derivative thus computes
\[
-\zeta_L'(0) = \log \det\nolimits_\zeta(L_{\mathrm{sym}}^2),
\]
in agreement with the Laplace representation in \lemref{lem:det_via_heat_trace}.
