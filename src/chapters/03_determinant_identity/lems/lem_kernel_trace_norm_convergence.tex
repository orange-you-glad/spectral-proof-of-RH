...
\medskip
\noindent
Then the mollified operators \( L_t \in \TC(H_{\Psi_\alpha}) \) (see \lemref{lem:trace_class_Lt}) converge to \( \Lsym \in \TC(H_{\Psi_\alpha}) \) in trace norm:
\[
\lim_{t \to 0^+} \| L_t - \Lsym \|_{\TC} = 0,
\]
as established more precisely in \lemref{lem:trace_norm_convergence_Lt_to_Lsym}, based on exponential decay of the kernels (see \lemref{lem:decay_mollified_kernel}).
