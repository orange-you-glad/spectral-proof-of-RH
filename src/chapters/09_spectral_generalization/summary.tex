...
\begin{itemize}
  ...
  \]
  matching the nontrivial zeros of \( L(s, \pi) \) with the nonzero spectrum of \( \Lsympi \), as shown in \thmref{thm:zero_encoding_general};

  \item A spectral equivalence:
  \[
  \GRH(\pi) \iff \Spec(\Lsympi) \subset \R,
  \]
  generalizing the RH \( \iff \) spectrum reality equivalence to the automorphic setting, as proven in \thmref{thm:spec_real_equiv_grh}.
\end{itemize}
